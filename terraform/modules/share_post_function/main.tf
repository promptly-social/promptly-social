...
# Storage bucket to hold the zipped code (reuse existing bucket from other functions)
...
resource "google_storage_bucket_object" "source_archive" {
  name   = "share-post-source.zip#${data.archive_file.source.output_md5}"
...
resource "google_service_account" "function_sa" {
  account_id   = "share-post-sa-${var.environment}"
...
# Data source for the GCP share post function URL secret
data "google_secret_manager_secret" "gcp_share_post_function_url" {
  secret_id = "GCP_SHARE_POST_FUNCTION_URL"
}

# Save the Cloud Function URL to Secret Manager
resource "google_secret_manager_secret_version" "gcp_share_post_function_url_version" {
  secret      = data.google_secret_manager_secret.gcp_share_post_function_url.id
...
# Grant the function's service account permission to write to the GCP share post function URL secret
resource "google_secret_manager_secret_iam_member" "secret_access_gcp_function_url" {
  project   = data.google_secret_manager_secret.gcp_share_post_function_url.project
  secret_id = data.google_secret_manager_secret.gcp_share_post_function_url.secret_id
...
# Allow Cloud Scheduler to invoke this function
