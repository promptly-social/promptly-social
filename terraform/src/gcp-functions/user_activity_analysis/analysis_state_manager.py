...
class AnalysisStateManager:
    ...
    async def get_last_analysis_timestamp(self, user_id: UUID) -> Optional[datetime]:
        ...
        try:
            results = await self.db_client.execute_query_async(
            ...
            logger.error(
                f"Error getting last analysis timestamp for user {user_id}: {e}"
            )
            raise
    ...
    async def record_analysis_completion(
        ...
        try:
            ...
            logger.info(
                f"Recorded analysis completion for user {user_id} at {analysis_timestamp}"
            )
    ...
    async def get_new_content_since_analysis(
        ...
        try:
            ...
            posts_result = await self.db_client.execute_query_async(
                posts_query, posts_params
            )
            ...
            messages_result = await self.db_client.execute_query_async(
                messages_query, messages_params
            )
    ...
    async def validate_analysis_state(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            tracking_results = await self.db_client.execute_query_async(
                tracking_query, {"user_id": str(user_id)}
            )
            tracking_result = tracking_results[0] if tracking_results else None

            if not tracking_result:
                validation_result["warnings"].append(
                    "No tracking record found for user"
                )
            ...
            if last_analysis_at and last_analysis_at < created_at:
                validation_result["is_valid"] = False
                validation_result["issues"].append(
                    "Last analysis timestamp is before record creation"
                )

            # Validate analysis scope if analysis has been completed
            if last_analysis_at and tracking_result["analysis_scope"]:
                try:
                    scope_validation = self._validate_analysis_scope(
                        tracking_result["analysis_scope"]
                    )
            ...
            if tracking_result["last_analyzed_post_id"]:
                ...
                if not post_exists:
                    validation_result["warnings"].append(
                        "Last analyzed post ID references non-existent post"
                    )
            ...
            if tracking_result["last_analyzed_message_id"]:
                ...
                message_exists_results = await self.db_client.execute_query_async(
                    message_exists_query,
                    {
                        "message_id": tracking_result["last_analyzed_message_id"],
                        "user_id": str(user_id),
                    },
                )
                message_exists = (
                    message_exists_results[0] if message_exists_results else None
                )
                if not message_exists:
                    validation_result["warnings"].append(
                        "Last analyzed message ID references non-existent message"
                    )

            # Validate content counts against actual database
            if last_analysis_at and tracking_result["analysis_scope"]:
                await self._validate_content_counts(
                    user_id, tracking_result["analysis_scope"], validation_result
                )
    ...
    async def cleanup_failed_analysis(self, user_id: UUID) -> None:
        ...
        try:
            rows_affected = await self.db_client.execute_update_async(
                query, {"user_id": str(user_id)}
            )
            ...
            if rows_affected > 0:
                ...
                logger.warning(
                    f"No tracking record found to clean up for user {user_id}"
                )
    ...
    async def get_analysis_progress_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "avg_hours_since_analysis": round(
                    result["avg_hours_since_analysis"] or 0, 2
                ),
    ...
    def _validate_analysis_scope(
        ...
        if "posts_analyzed" in analysis_scope:
            ...
            if not isinstance(posts_data, dict):
                validation_result["is_valid"] = False
                validation_result["issues"].append(
                    "posts_analyzed must be a dictionary"
                )
            else:
                ...
                for key in required_post_keys:
                    if key not in posts_data:
                        validation_result["is_valid"] = False
                        validation_result["issues"].append(
                            f"Missing posts_analyzed key: {key}"
                        )
                    elif not isinstance(posts_data[key], int) or posts_data[key] < 0:
                        validation_result["is_valid"] = False
                        validation_result["issues"].append(
                            f"posts_analyzed.{key} must be a non-negative integer"
                        )
        ...
        if "messages_analyzed" in analysis_scope:
            ...
            if not isinstance(messages_data, dict):
                validation_result["is_valid"] = False
                validation_result["issues"].append(
                    "messages_analyzed must be a dictionary"
                )
            else:
                if "total_count" not in messages_data:
                    validation_result["is_valid"] = False
                    validation_result["issues"].append(
                        "Missing messages_analyzed key: total_count"
                    )
                elif (
                    not isinstance(messages_data["total_count"], int)
                    or messages_data["total_count"] < 0
                ):
                    validation_result["is_valid"] = False
                    validation_result["issues"].append(
                        "messages_analyzed.total_count must be a non-negative integer"
                    )
        ...
        if "analysis_types_performed" in analysis_scope:
            ...
            if not isinstance(analysis_types, list):
                validation_result["is_valid"] = False
                validation_result["issues"].append(
                    "analysis_types_performed must be a list"
                )
            else:
                valid_types = [
                    "writing_style",
    ...
    async def _validate_content_counts(
        ...
        try:
            ...
            posts_results = await self.db_client.execute_query_async(
                posts_query, {"user_id": str(user_id)}
            )
            posts_result = posts_results[0] if posts_results else None

            if posts_result and "posts_analyzed" in analysis_scope:
                recorded_scheduled = analysis_scope["posts_analyzed"].get(
                    "scheduled_count", 0
                )
                recorded_dismissed = analysis_scope["posts_analyzed"].get(
                    "dismissed_count", 0
                )
            ...
            messages_results = await self.db_client.execute_query_async(
                messages_query, {"user_id": str(user_id)}
            )
            ...
            if messages_result and "messages_analyzed" in analysis_scope:
                recorded_messages = analysis_scope["messages_analyzed"].get(
                    "total_count", 0
                )
    ...
    async def record_analysis_progress(
        ...
        try:
            await self.db_client.execute_update_async(
                query,
                {"user_id": str(user_id), "progress_scope": json.dumps(extended_scope)},
            )

            logger.debug(
                f"Recorded analysis progress for user {user_id}: {progress_data['current_operation']}"
            )
    ...
    async def detect_interrupted_analysis(
        ...
        try:
            results = await self.db_client.execute_query_async(
                query, {"timeout_threshold": timeout_threshold}
            )

            interrupted_analyses = []
            for row in results:
                progress_data = (
                    row["analysis_scope"].get("progress", {})
                    if row["analysis_scope"]
                    else {}
                )
    ...
    async def recover_interrupted_analysis(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            if rows_affected > 0:
                ...
                logger.warning(
                    f"No interrupted analysis found to recover for user {user_id}"
                )
            ...
            logger.error(
                f"Error recovering interrupted analysis for user {user_id}: {e}"
            )
    ...
    async def get_analysis_recovery_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "avg_minutes_since_update": round(
                    result["avg_minutes_since_update"] or 0, 2
                ),
            }
    ...
    async def batch_recover_interrupted_analyses(
        self, timeout_minutes: int = 60, max_recoveries: int = 100
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            interrupted_analyses = await self.detect_interrupted_analysis(
                timeout_minutes
            )
            ...
            for analysis_info in analyses_to_recover:
                try:
                    recovery_result = await self.recover_interrupted_analysis(
                        analysis_info["user_id"]
                    )
                    ...
                    batch_results["recovery_details"].append(
                        {
                            ...
                            "minutes_interrupted": analysis_info[
                                "minutes_since_update"
                            ],
                            "previous_progress": recovery_result.get(
                                "previous_progress"
                            ),
                        }
                    )
                    ...
                    logger.error(
                        f"Failed to recover analysis for user {analysis_info['user_id']}: {e}"
                    )
