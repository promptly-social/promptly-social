...
class AnalysisDatabaseClient:
    ...
    async def batch_execute_updates(
        self, queries_and_params: List[Tuple[str, Dict[str, Any]]]
    ) -> List[int]:
        """
    ...
    async def get_user_analysis_summary(self, user_id: UUID) -> Dict[str, Any]:
        ...
        query = """
            SELECT 
                ...
                ws.analysis_data as writing_style_analysis,
            ...
            LEFT JOIN writing_style_analysis ws ON u.id = ws.user_id
        ...
        if summary.get("topics_of_interest"):
            try:
                summary["topics_of_interest"] = (
                    json.loads(summary["topics_of_interest"])
                    if isinstance(summary["topics_of_interest"], str)
                    else summary["topics_of_interest"]
                )
            except (json.JSONDecodeError, TypeError):
                summary["topics_of_interest"] = []
    ...
    async def update_user_analysis_results(
        ...
        if "bio_update" in analysis_results:
            ...
            queries_and_params.append(
                (
                    query,
                    {"user_id": str(user_id), "bio": analysis_results["bio_update"]},
                )
            )
        ...
        queries_and_params.append(
            (
                tracking_query,
                {"user_id": str(user_id), "analysis_scope": json.dumps(analysis_scope)},
            )
        )
        ...
        logger.info(
            f"Updated analysis results for user {user_id}: {list(analysis_results.keys())}"
        )
    ...
    async def get_users_needing_analysis_batch(
        ...
        logger.info(
            f"Retrieved {len(results)} users needing analysis (batch_size={batch_size}, offset={offset})"
        )
        return results
    ...
    async def get_analysis_performance_metrics(self) -> Dict[str, Any]:
        ...
        metrics.update({"connection_pool": self._connection_pool_stats.copy()})
