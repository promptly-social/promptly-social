        ...
        try:
            while self._monitoring:
                ...
                self.memory_samples.append(
                    {"timestamp": time.time(), "memory_mb": memory_mb}
                )
...
class AdaptiveBatchProcessor:
    ...
    async def process_users_adaptively(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Batch processing timed out after {timeout_seconds} seconds"
                )
    ...
    def _adjust_batch_size(self):
        ...
        avg_throughput = sum(
            m.throughput_users_per_second for m in recent_metrics
        ) / len(recent_metrics)
        ...
        if avg_memory > self.thresholds.memory_warning_threshold_mb:
            ...
            logger.info(
                f"Reduced batch size to {self.current_batch_size} due to high memory usage"
            )

        # Adjust based on throughput and success rate
        elif (
            avg_throughput > self.thresholds.min_throughput_users_per_second
            and avg_success_rate > 0.9
        ):
            self.current_batch_size = min(
                self.max_batch_size, int(self.current_batch_size * 1.2)
            )
            logger.info(
                f"Increased batch size to {self.current_batch_size} due to good performance"
            )
    ...
    def _adjust_concurrency(self, avg_memory: float, avg_throughput: float):
        ...
        if avg_memory > self.thresholds.memory_warning_threshold_mb:
            ...
            logger.info(
                f"Reduced concurrency to {self.current_concurrency} due to high memory usage"
            )
        elif (
            avg_throughput > self.thresholds.min_throughput_users_per_second
            and avg_memory < self.thresholds.max_memory_mb * 0.7
        ):
            self.current_concurrency = min(
                self.max_concurrency, self.current_concurrency + 1
            )
            logger.info(
                f"Increased concurrency to {self.current_concurrency} due to good performance"
            )
        ...
        async with semaphore:
            ...
            try:
                ...
                successful_results = sum(
                    1 for r in results if r.status == AnalysisStatus.SUCCESS
                )
                ...
                metrics = BatchMetrics(
                    ...
                    throughput_users_per_second=len(batch) / processing_time
    ...
    def _check_performance_warnings(self, metrics: BatchMetrics, batch_index: int):
        ...
        if (
            metrics.throughput_users_per_second
            < self.thresholds.min_throughput_users_per_second
        ):
            warnings.append(
                f"Low throughput: {metrics.throughput_users_per_second:.2f} users/sec"
            )
        ...
        if warnings:
            logger.warning(
                f"Performance warnings for batch {batch_index}: {'; '.join(warnings)}"
            )

    def _log_performance_summary(
        self, total_users: int, results_count: int, total_time: float
    ):
        ...
        avg_memory = sum(m.memory_usage_mb for m in self.batch_history) / len(
            self.batch_history
        )
        peak_memory = max(m.peak_memory_mb for m in self.batch_history)
        avg_throughput = sum(
            m.throughput_users_per_second for m in self.batch_history
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_throughput_users_per_second": sum(
                m.throughput_users_per_second for m in recent_metrics
            )
            / len(recent_metrics),
    ...
    async def shutdown(self, timeout_seconds: int = 30):
        ...
        if self.active_tasks:
            try:
                ...
                logger.warning(
                    f"Some tasks did not complete within {timeout_seconds}s, cancelling..."
                )
