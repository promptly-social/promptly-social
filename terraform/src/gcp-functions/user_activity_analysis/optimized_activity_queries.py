...
class OptimizedActivityQueries:
    ...
    @asynccontextmanager
    async def _monitor_query_performance(
        self, query_name: str, query: str, params: List = None
    ):
        """
        ...
        try:
            ...
            if execution_time > 0.1:
                ...
                logger.debug(f"Query {query_name} completed in {execution_time:.3f}s")
    ...
    async def get_optimized_post_counts(
        ...
        async with self._monitor_query_performance(
            "get_optimized_post_counts", optimized_query, params
        ):
    ...
    async def get_optimized_message_counts(
        ...
        if since_timestamp:
            conditions.extend(["c.created_at > %s", "m.created_at > %s"])
        ...
        async with self._monitor_query_performance(
            "get_optimized_message_counts", optimized_query, params
        ):
            result = await self.db_client.fetch_one(optimized_query, params)
    ...
    async def get_batch_user_activity_counts(
        ...
        async with self._monitor_query_performance(
            "batch_post_counts", batch_post_query, post_params
        ):
        ...
        if since_timestamps:
            ...
            if message_timestamp_conditions:
                message_conditions.append(
                    f"({' OR '.join(message_timestamp_conditions)})"
                )
        ...
        async with self._monitor_query_performance(
            "batch_message_counts", batch_message_query, message_params
        ):
            message_results = await self.db_client.fetch_all(
                batch_message_query, message_params
            )
    ...
    async def get_users_analysis_tracking_batch(
        ...
        async with self._monitor_query_performance(
            "get_users_analysis_tracking_batch", tracking_query, params
        ):
    ...
    async def get_content_for_analysis_batch(
        self, user_activity_data: Dict[UUID, Dict[str, Any]]
        ...
        if user_conditions:
            ...
            async with self._monitor_query_performance(
                "get_content_posts_batch", posts_query
            ):
        ...
        if message_conditions:
            ...
            async with self._monitor_query_performance(
                "get_content_messages_batch", messages_query
            ):
    ...
    async def update_analysis_tracking_batch(
        self, tracking_updates: List[Dict[str, Any]]
    ) -> None:
        """
        ...
        for update in tracking_updates:
            batch_values.append(
                (
                    ...
                    str(update.get("last_post_id"))
                    if update.get("last_post_id")
                    else None,
                    str(update.get("last_message_id"))
                    if update.get("last_message_id")
                    else None,
        ...
        async with self._monitor_query_performance(
            "update_analysis_tracking_batch", upsert_query
        ):
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        slow_queries = [
            q for q in self.query_performance_log if q["execution_time_ms"] > 100
        ]
    ...
    async def validate_indexes(self) -> Dict[str, bool]:
        ...
        async with self._monitor_query_performance(
            "validate_indexes", index_check_query, [required_indexes]
        ):
            existing_indexes = await self.db_client.fetch_all(
                index_check_query, [required_indexes]
            )
