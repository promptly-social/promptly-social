...
class ActivityThresholdChecker:
    ...
    async def check_user_activity(
        self, user_id: UUID, last_analysis_at: Optional[datetime] = None
    ) -> Dict[str, int]:
        """
        ...
        meets_threshold = (
            total_posts >= self.post_threshold
    ...
    async def get_message_counts(
        ...
        conversations = await self.db_client.fetch_all(
            conversations_query, conversation_params
        )
    ...
    async def get_users_needing_analysis(
        ...
        post_thresh = (
            post_threshold if post_threshold is not None else self.post_threshold
        )
        message_thresh = (
            message_threshold
            if message_threshold is not None
            else self.message_threshold
        )
        ...
        for record in tracking_records:
            ...
            # Check if user meets thresholds
            if (
                activity["total_posts"] >= post_thresh
                or activity["messages"] >= message_thresh
            ):
    ...
    async def get_batch_analysis_summary(
        ...
        post_triggered = sum(
            1
            for _, activity in users_needing_analysis
        ...
        both_triggered = sum(
            ...
            for _, activity in users_needing_analysis
            if (
                activity["total_posts"] >= (post_threshold or self.post_threshold)
                and activity["messages"]
                >= (message_threshold or self.message_threshold)
            )
        )

        # Calculate activity totals
        total_posts = sum(
            activity["total_posts"] for _, activity in users_needing_analysis
        )
        total_messages = sum(
            activity["messages"] for _, activity in users_needing_analysis
        )
