...
from user_activity_analysis.config import ConfigManager
...
def validate_request(request) -> Dict[str, Any]:
    ...
    if not isinstance(config['message_threshold'], int) or config['message_threshold'] < 1:
        raise ValueError("message_threshold must be a positive integer")
    
    if not isinstance(config['batch_timeout_minutes'], int) or config['batch_timeout_minutes'] < 1:
        raise ValueError("batch_timeout_minutes must be a positive integer")
    
    if not isinstance(config['max_users_per_batch'], int) or config['max_users_per_batch'] < 1:
...
def create_error_response(error_message: str, error_type: str = "analysis_error") -> Dict[str, Any]:
    """
...
def user_activity_analysis(request):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"User activity analyzer initialized with thresholds: posts={config['post_threshold']}, messages={config['message_threshold']}")
        except Exception as e:
            logger.error(f"Failed to initialize analyzer: {e}")
            return (
                json.dumps(create_error_response(
                    f"Analyzer initialization failed: {str(e)}",
                    "analyzer_error"
                )),
                500,
                headers
        ...
        try:
            ...
            analysis_execution = execute_analysis_with_timeout(
                analyzer,
                config['batch_timeout_minutes']
            )
            
            execution_result = asyncio.run(analysis_execution)
            ...
            logger.error(f"Critical error during analysis execution: {e}", exc_info=True)
            return (
                json.dumps(create_error_response(
        ...
        logger.error(f"Unexpected error in user activity analysis function: {e}", exc_info=True)
        ...
        # Perform cleanup
        asyncio.run(cleanup_resources(db_client, analyzer))
...
def health_check(request):
    ...
    try:
        ...
        response = {
            ...
            'database': db_status,
            'configuration': {
                'ai_provider': config_manager.ai_provider,
