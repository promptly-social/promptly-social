...
class OpenAIProvider(AIProviderInterface):
    ...
    def __init__(self, config: ProviderConfig):
        ...
        if config.provider_type != AIProvider.OPENAI:
            raise ValueError(f"Invalid provider type for OpenAI: {config.provider_type}")
        ...
        self.base_url = config.additional_config.get('base_url', 'https://api.openai.com/v1')
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if 'quota' in error_message.lower():
                            raise AIServiceQuotaError(f"OpenAI quota exceeded: {error_message}")
                        else:
                            raise AIServiceRateLimitError(f"OpenAI rate limit: {error_message}", retry_after_int)
                    
                    elif response.status in [400, 401, 403]:
                        error_data = await response.json()
                        error_message = error_data.get('error', {}).get('message', 'API error')
                        raise AIServiceError(f"OpenAI API error ({response.status}): {error_message}")
                    
                    else:
                        raise AIServiceError(f"OpenAI API error: HTTP {response.status}")
        
        except asyncio.TimeoutError:
            raise AIServiceTimeoutError(f"OpenAI request timed out after {self.timeout} seconds")
...
Return the results as a valid JSON array."""
        ...
        try:
            ...
            logger.warning(f"Failed to parse JSON from OpenAI topics response: {response}")
