...
class UserActivityAnalyzer:
    ...
    def __init__(
        ...
        Args:
            ...
            ai_service: AI analysis service (created if None)
        ...
        try:
            ...
            final_result = BatchAnalysisResult(
                ...
                successful_analyses=sum(
                    1 for r in batch_results if r.status == AnalysisStatus.SUCCESS
                ),
                failed_analyses=sum(
                    1 for r in batch_results if r.status == AnalysisStatus.FAILED
                ),
                skipped_analyses=sum(
                    1 for r in batch_results if r.status == AnalysisStatus.SKIPPED
                ),
            ...
            logger.error(
                f"Critical error in user activity analysis: {e}", exc_info=True
            )
            ...
            return BatchAnalysisResult(
                ...
                skipped_analyses=sum(
                    1
                    for r in self.current_batch_results
                    if r.status == AnalysisStatus.SKIPPED
                ),
                total_processing_time_seconds=processing_time,
                start_time=self.analysis_start_time,
                end_time=end_time,
    ...
    async def get_users_for_analysis(self) -> List[Dict[str, Any]]:
        ...
        try:
            users_needing_analysis = (
                await self.state_manager.get_users_needing_analysis(
                    self.post_threshold, self.message_threshold
                )
            )

            logger.info(
                f"Threshold check found {len(users_needing_analysis)} users needing analysis"
            )

            # Log breakdown by trigger type
            post_triggered = sum(
                1
                for user in users_needing_analysis
                if user["post_count"] >= self.post_threshold
            )
            message_triggered = sum(
                1
                for user in users_needing_analysis
                if user["message_count"] >= self.message_threshold
            )
            both_triggered = sum(
                1
                for user in users_needing_analysis
                if user["post_count"] >= self.post_threshold
                and user["message_count"] >= self.message_threshold
            )

            logger.info(
                f"Analysis triggers: {post_triggered} by posts, {message_triggered} by messages, {both_triggered} by both"
            )
    ...
    async def process_user_batch(
        ...
        async def process_single_user_with_semaphore(
        ...
        for i, task in enumerate(asyncio.as_completed(tasks)):
            try:
                ...
                logger.error(
                    f"Unexpected error in batch processing: {e}", exc_info=True
                )
    ...
    async def analyze_single_user(
        ...
        try:
            ...
            analysis_results = await self._orchestrate_ai_analysis(
            ...
            await self.update_analysis_tracking(
                user_id,
                {
            ...
            logger.info(
                f"Successfully completed analysis for user {user_id} in {processing_time:.2f}s. Types: {analysis_types}"
            )
            ...
            logger.error(
                f"Analysis failed for user {user_id} ({email}): {error_message}",
                exc_info=True,
            )
            ...
            try:
                ...
                logger.error(
                    f"Failed to cleanup analysis state for user {user_id}: {cleanup_error}"
                )
        ...
        try:
            ...
            for post in content["posts"]:
                if post["status"] in ["scheduled", "posted"]:
                    ...
                    or post.get("user_feedback") == "negative"
                ):
    ...
    async def _get_user_profile_data(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            user_results = await self.db_client.execute_query_async(
                user_query, {"user_id": str(user_id)}
            )
            ...
            preferences_query = """
                SELECT bio, negative_analysis
                FROM user_preferences
                WHERE user_id = :user_id
            """

            pref_results = await self.db_client.execute_query_async(
                preferences_query, {"user_id": str(user_id)}
            )
            ...
            style_results = await self.db_client.execute_query_async(
                writing_style_query, {"user_id": str(user_id)}
            )
            writing_style = style_results[0].get("analysis_data") if style_results else ""
    ...
    def _has_sufficient_content(self, content_data: Dict[str, Any]) -> bool:
        ...
        return (has_posts or has_messages) and (
            has_scheduled_for_writing_analysis
            or has_dismissed_for_negative_analysis
            or has_messages >= 5
        )

    async def _orchestrate_ai_analysis(
        self, user_id: UUID, content_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if len(content_data.get("scheduled_posts", [])) >= 2:
                ...
                scheduled_content = [
                    post["content"] for post in content_data["scheduled_posts"]
                ]
                ...
                writing_style_result = await self.ai_service.analyze_writing_style(
                    content_list=scheduled_content, existing_analysis=existing_analysis
                )
            ...
            if len(content_data.get("scheduled_posts", [])) >= 1:
                ...
                topics_result = await self.ai_service.analyze_topics_of_interest(
                    scheduled_content
                )
            ...
            if len(content_data.get("scheduled_posts", [])) >= 1 and user_profile.get(
                "bio"
            ):
                ...
                bio_result = await self.ai_service.update_user_bio(
                    current_bio=current_bio, recent_content=scheduled_content
                )
            ...
            if len(content_data.get("dismissed_posts", [])) >= 1:
                ...
                negative_result = await self.ai_service.analyze_negative_patterns(
                    ...
                    scheduled_posts=scheduled_content,
                    current_analysis=user_profile.get("negative_analysis")
                )
            ...
            logger.info(
                f"AI analysis orchestration completed for user {user_id}. Analysis types: {list(analysis_results.keys())}"
            )
    ...
    async def _store_analysis_results(
        self, user_id: UUID, analysis_results: Dict[str, Any]
    ) -> None:
        """
        ...
        try:
            ...
            if "writing_style" in analysis_results:
                await self._update_writing_style_analysis(
                    user_id, analysis_results["writing_style"]
                )

            # Update topics of interest if present
            if "topics_of_interest" in analysis_results:
                await self._update_topics_of_interest(
                    user_id, analysis_results["topics_of_interest"]
                )
            ...
            if "negative_analysis" in analysis_results:
                await self._update_negative_analysis(
                    user_id, analysis_results["negative_analysis"]
                )
    ...
    async def _update_writing_style_analysis(
        self, user_id: UUID, writing_style_result: str
    ) -> None:
        """Update writing style analysis in user preferences."""
        query = """
            INSERT INTO writing_style_analysis (user_id, analysis_data, updated_at)
        ...
        await self.db_client.execute_update_async(
            query,
            {"user_id": str(user_id), "writing_style_analysis": writing_style_result},
        )

    async def _update_topics_of_interest(
        self, user_id: UUID, topics_result: List[Dict[str, Any]]
    ) -> None:
        ...
        logger.debug(
            f"Topics analysis result for user {user_id}: {len(topics_result)} topics identified"
        )
        topics = [result.topic for result in topics_result]
        ...
        existing_topics = await self.db_client.execute_query_async(
            query, {"user_id": str(user_id)}
        )

        existing_topics = existing_topics[0].get("topics_of_interest", []) if existing_topics else []

        updated_topics = list(set(existing_topics + topics))

        query = """
            UPDATE user_preferences
            SET topics_of_interest = :topics_of_interest, updated_at = NOW()
            WHERE user_id = :user_id
        """

        await self.db_client.execute_update_async(
            query, {"user_id": str(user_id), "topics_of_interest": updated_topics}
        )
    ...
    async def _update_user_bio(self, user_id: UUID, bio_result: str) -> None:
        ...
        await self.db_client.execute_update_async(
            query, {"user_id": str(user_id), "bio": bio_result}
        )

    async def _update_negative_analysis(
        self, user_id: UUID, negative_result: str
    ) -> None:
        ...
        await self.db_client.execute_update_async(
            query, {"user_id": str(user_id), "negative_analysis": negative_result}
        )

    def _create_analysis_scope(
        self, content_data: Dict[str, Any], analysis_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        return {
            "posts_analyzed": {
                ...
                "date_range": {
                    ...
                    if content_data["posts"]
                    and any("created_at" in post for post in content_data["posts"])
                    else None,
                    "end": max(
                        post["created_at"]
                        for post in content_data["posts"]
            ...
            "messages_analyzed": {
                ...
                "date_range": {
                    "start": min(
                        msg["created_at"]
                        for msg in content_data["messages"]
                        if "created_at" in msg
                    ).isoformat()
                    if content_data["messages"]
                    and any("created_at" in msg for msg in content_data["messages"])
                    else None,
                    "end": max(
                        msg["created_at"]
                        for msg in content_data["messages"]
    ...
    async def update_analysis_tracking(
        self, user_id: UUID, analysis_data: Dict[str, Any]
    ) -> None:
        """
        ...
        try:
            ...
            logger.error(
                f"Error updating analysis tracking for user {user_id}: {e}",
                exc_info=True,
            )
            raise
    ...
    async def _process_with_timeout(
        ...
        try:
            ...
            results = await asyncio.wait_for(
                self.process_user_batch(users), timeout=self.batch_timeout_seconds
            )
            return results
            ...
            processed_user_ids = {
                result.user_id for result in self.current_batch_results
            }
    ...
    def _generate_error_summary(
        self, results: List[UserAnalysisResult]
    ) -> Dict[str, int]:
        """
        ...
        for result in results:
            if result.status != AnalysisStatus.SUCCESS and result.error_message:
                ...
                if (
                    "ai service" in error_msg
                    or "openai" in error_msg
                    or "anthropic" in error_msg
                    or ("ai" in error_msg and "service" in error_msg)
                ):
    ...
    def _log_batch_summary(self, result: BatchAnalysisResult) -> None:
        ...
        if result.total_users_processed > 0:
            success_rate = (
                result.successful_analyses / result.total_users_processed
            ) * 100
        ...
        if result.successful_analyses > 0:
            avg_processing_time = (
                result.total_processing_time_seconds / result.total_users_processed
            )
            logger.info(
                f"Average processing time per user: {avg_processing_time:.2f} seconds"
            )
        ...
        for user_result in result.user_results:
            if user_result.status == AnalysisStatus.SUCCESS:
                for analysis_type in user_result.analysis_types_performed:
                    analysis_type_counts[analysis_type] = (
                        analysis_type_counts.get(analysis_type, 0) + 1
                    )
        ...
        failed_users = [
            r for r in result.user_results if r.status == AnalysisStatus.FAILED
        ]
        if failed_users:
            logger.warning(
                f"Failed analyses for users: {[str(r.user_id) for r in failed_users[:10]]}"
            )  # Limit to first 10
        ...
        return {
            "summary": {
                ...
                "success_rate_percent": (
                    result.successful_analyses / result.total_users_processed * 100
                )
                if result.total_users_processed > 0
                else 0,
                "total_processing_time_seconds": result.total_processing_time_seconds,
                "average_processing_time_seconds": result.total_processing_time_seconds
            ...
            "analysis_types_performed": self._get_analysis_type_summary(
                result.user_results
            ),
        }

    def _get_analysis_type_summary(
        self, user_results: List[UserAnalysisResult]
    ) -> Dict[str, int]:
